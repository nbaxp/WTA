...
public class GenericWebController<TEntity, TDisplayModel, TEditModel> : Controller
  where TEntity : class
  ...
  where TEditModel : class
{
  ...
  public GenericWebController(IRepository<TEntity> repository)
  {
  ...
  public async Task<IActionResult> Index([FromQuery] PaginationViewModel<TEntity> model)
  {
    try
    {
      ...
      {
        var query = this._repository.NoTracking();
