...
public class DefaultDbSeed : IDbSeed
{
    ...
    public async Task Seed()
    {
        try
        {
            ...
            await _db.Set<Role>().AddAsync(superRole).ConfigureAwait(false);
            await _db.Set<Role>().AddAsync(adminRole).ConfigureAwait(false);
            var super = new User
            {
                UserName = "super",
            ...
            await _db.Set<User>().AddAsync(super).ConfigureAwait(false);
            await _db.Set<User>().AddAsync(admin).ConfigureAwait(false);
