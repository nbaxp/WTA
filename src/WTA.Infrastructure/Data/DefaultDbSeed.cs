...
public class DefaultDbSeed : IDbSeed
{
    ...
    public void Initialize()
    {
        ...
        {
            throw new Exception($"数据库初始化数据异常:{ex.Message}", ex);
        }
    }

    private static void InitSystem(IStringLocalizer _localizer, DbContext _db)
    {
        //添加权限
        ///默认用户super、admin
        ...
        foreach (var item in resourceTypes)
        {
            ...
            var resourceName = _localizer[item.GetCustomAttribute<DisplayAttribute>()?.Name ?? item.FullName!];
            ...
            if (permissions != null && permissions.Permissions != OperationType.None)
            {
                foreach (var operationName in Enum.GetNames(typeof(OperationType)))
                {
                    ...
                    if (permissions.Permissions.HasFlag(operation))
                    {
                        ...
                        var name = _localizer[operation.GetDisplayName()];
    ...
    private static void InitUsers(IPasswordHasher _passwodHasher, DbContext _db)
    {
        ...
        _db.Set<User>().Add(super);
        _db.Set<User>().Add(admin);
        _db.SaveChanges();
    }

    private static void InitBlog(DbContext _db)
    {
        // Blog
        _db.Set<BlogPost>().Add(new BlogPost
        {
